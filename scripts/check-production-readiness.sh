...
require_absent_text "web/src/views/LoginView.vue" '数据本地加密存储|安全无忧'
require_text "web/src/views/LoginView.vue" '数据由你自行托管，敏感凭据加密保存'
require_text "web/src/views/SettingsView.vue" '文件未加密，请妥善保管'
require_text "mobile/lib/features/data_management/presentation/data_management_page.dart" '文件未加密，请妥善保管'
...
require_text "scripts/check-release-artifact-files.sh" 'awk.*END.*NR'
...
require_text "scripts/check-docker-release-evidence.sh" 'json.load'
require_text "scripts/check-docker-release-evidence.sh" 'linux/amd64'
require_text "scripts/check-docker-release-evidence.sh" 'linux/arm64'
require_text "scripts/check-docker-release-evidence.sh" 'Docker manifest missing required linux platforms'
require_text "scripts/check-docker-release-evidence.sh" 'local smoke_status="[$][?]"'
require_text "scripts/check-docker-release-evidence.sh" 'pull=never --network none --read-only'
require_text "scripts/check-docker-release-evidence.sh" 'user 0:0 --entrypoint /bin/sh'
require_text "scripts/check-docker-release-evidence.sh" 'smoke_dir/data:/cleanup'
require_text "scripts/check-docker-release-evidence.sh" 'rm -rf /cleanup/[*]'
require_text "scripts/check-docker-release-evidence.sh" 'Compose cleanup failed'
require_text "scripts/check-docker-release-evidence.sh" 'data cleanup failed'
require_text "scripts/check-docker-release-evidence.sh" 'cleanup left temporary data'
require_text "scripts/check-docker-release-evidence.sh" 'exit "[$]smoke_status"'
...
require_text "scripts/check-final-release-gates.sh" 'env RELEASE_VERSION=.*REQUIRE_IOS_ARTIFACT=1 VERIFY_ARTIFACT_SIGNATURES=1 "\$ROOT_DIR/scripts/check-release-artifact-files\.sh"'
require_text "scripts/check-final-release-gates.sh" 'REQUIRE_ANDROID_EMULATOR=1'
require_text "scripts/check-final-release-gates.sh" 'RUN_PHYSICAL_IOS_E2E=1'
require_text "scripts/check-final-release-gates.sh" 'RUN_ANDROID_E2E=1'
require_text "scripts/check-final-release-gates.sh" 'RELEASE_SCOPE:-signed-mobile'
require_text "scripts/check-final-release-gates.sh" 'RELEASE_PHASE:-source'
require_text "scripts/check-final-release-gates.sh" 'Docker/Web .* release gate checks passed'
require_text "scripts/check-final-release-gates.sh" 'RUNTIME_DOCKER_RELEASE_EVIDENCE=1'
require_text "scripts/check-docker-release-evidence.sh" 'RUNTIME_DOCKER_RELEASE_EVIDENCE:-0'
require_text ".github/workflows/release-web.yml" 'Docker/Web source release contract'
require_text ".github/workflows/release-web.yml" 'Published Docker image runtime gate'
...
require_text "scripts/check-mobile-device-qa-preflight.sh" 'ANDROID_EMULATOR_BOOT_TIMEOUT:-300'
require_text "scripts/verify-mobile-e2e.sh" 'ANDROID_EMULATOR_BOOT_TIMEOUT:-300'
require_text "scripts/check-mobile-device-qa-preflight.sh" 'nohup "[$]emulator_bin"'
require_text "mobile/QA/check_device_readiness.sh" 'nohup "[$]emulator_bin"'
require_text "scripts/install-android-profile.sh" 'nohup "[$]emulator_bin"'
...
require_text "docs/quality/release-artifact-evidence-2026-05-27.md" 'Each `[.]sha256` sidecar must contain exactly one checksum line'
...
require_text "docs/quality/final-release-runbook-2026-05-27.md" 'DOCKER_RELEASE_IMAGE=ghcr.io/sky121666/sky-personalledger:1\.0\.9'
