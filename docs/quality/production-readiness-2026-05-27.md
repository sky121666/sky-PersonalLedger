...
| Mobile device QA preflight | Current device list is inspectable, wireless iPhone is distinguished from USB iPhone, Android device/emulator presence is checkable, and physical/E2E commands are fixed | PREPARED | `./scripts/check-mobile-device-qa-preflight.sh`; use `REQUIRE_PHYSICAL_IOS=1 REQUIRE_ANDROID_DEVICE=1` for release |
