...
| Physical device proof | Real iPhone and Android emulator platform checks | PARTIAL; USB iPhone and Android emulator evidence remain missing |
...
| Mobile device QA | USB-connected iPhone and Android emulator run smoke or install/manual checklist | MISSING | `REQUIRE_PHYSICAL_IOS=1 REQUIRE_ANDROID_EMULATOR=1 ./scripts/check-mobile-device-qa-preflight.sh` |
...
| Mobile device QA preflight | Current device list is inspectable, wireless iPhone is distinguished from USB iPhone, Android emulator presence is checkable, and physical/E2E commands are fixed | PREPARED | `./scripts/check-mobile-device-qa-preflight.sh`; use `REQUIRE_PHYSICAL_IOS=1 REQUIRE_ANDROID_EMULATOR=1` for release |
...
| --- | --- | --- | --- |
| P0 | Physical iPhone is wireless-only and Android emulator evidence is not recorded in current evidence | Cannot prove real iOS install, Android install, safe area, keyboard, haptics, and performance | Connect iPhone by USB, start Android emulator, then run app smoke plus premium manual checklist |
