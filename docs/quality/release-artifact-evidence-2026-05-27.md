...
Each `.sha256` sidecar must contain exactly one checksum line in the default `shasum -a 256 <artifact>` format. Do not add blank lines. The filename in that line must be the artifact basename, not an absolute path, relative path, or a different artifact name.
