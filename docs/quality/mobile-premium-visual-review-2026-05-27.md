...
| P2 | Physical iPhone and manual device QA | Current visual evidence does not include a USB-connected iPhone run, maximum text size, weak-network recovery, or assistive-technology review. Android native real-backend E2E passed in GitHub Actions run `30776159138`. | OPEN | Requires USB iPhone execution and the remaining manual checklist before public distribution. |
...
1. Run the same premium flows on a USB-connected iPhone and complete the remaining manual Android checklist before public distribution.
