...
This review does not replace final physical-device QA. It verifies visual structure, readability, and obvious overflow issues from the current screenshot set.
...
| P2 | Physical iPhone | Wireless-only device cannot run Flutter integration tests in this session. | OPEN | Requires USB connection or another supported published-port workflow. |
...
| Safe area | Top bars and bottom action areas are visible in the exported frames. Physical devices still need final safe-area confirmation. |
...
1. Run the same premium flows on a physical iPhone once USB access is available.
...
3. Run a real screen-reader pass before public distribution if VoiceOver/TalkBack quality is a release criterion.
