...
- AI provider configuration unless an encrypted export/import strategy is explicitly added later
...
Restore replaces the user-owned ledger domain rows listed above for the selected user. It must not overwrite password hashes, login lock state, refresh tokens, API tokens, AI provider secrets, AI provider configuration, system settings, or uploaded file bytes.
